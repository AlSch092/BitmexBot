...
namespace BitMexBot
{
    public class position
    {
        ...
        public static position GetOpenPosition(Account A, string Symbol, int Count, string columns) //fix this
        {
            BitMEXApi bitmex = new BitMEXApi(A.APIKey, A.SecretKey);

            string TargetURI = "/position?filter=%7B%22symbol%22%3A%22" + Symbol + "%22%7D&columns=" + columns + "&count=1";
            ...
            else
            {
                ...
                try
                {
                    data = JsonConvert.DeserializeObject<position>(Result);
                }
                catch
                {
                    Console.WriteLine("Failed to deserialize position data.");
                }
