...
namespace BitMexBot
{
    public class Transaction //uses json dict
    {
        ...
        public bool CancelTransaction(Account A, string TransactionID) //we can bulk delete by adding more params.
        {
         ...
         public bool LimitTransaction(Account A, bool execInst, uint orderQty, OrderTypes OrdType, double price, string Symbol, string BuyOrSell)
         {
            BitMEXApi bitmex = new BitMEXApi(A.APIKey, A.SecretKey);
            ...
            {
                Params.Add("price", price.ToString().Replace("\"", ""));
                Params.Add("execInst", "ParticipateDoNotInitiate");
            }
            ...
            try
            {
                ...
                Console.WriteLine(data.account + " " +  data.symbol +" " +  " " + data.side);
            }
            catch
            {
        ...
        public bool MarketTransaction(Account A, uint Quantity, string Side, string Symbol) //market order chooses price for u, highest by default
        {
            ...
            try
            {
                if (Side == "Sell")
                {
                    ...
                    bool Result = S.LimitTransaction(A, true, Quantity, Transaction.OrderTypes.Market, Convert.ToDouble(0), Symbol, "Sell");

                }
                else if (Side == "Buy")
                {
                    Transaction L = new Transaction();
                    L.LimitTransaction(A, true, Quantity, Transaction.OrderTypes.Market, Convert.ToDouble(0), Symbol, "Buy");
